  ...
  return (
    <>
      ...
      <div className="relative w-full max-w-[1120px] mx-auto p-6 bg-gradient-to-b from-[#953bf562] to-[#0A0612] rounded-lg text-white">
        ...
        <button
          onClick={scrollLeft}
          className="absolute left-2 top-1/2 -translate-y-1/2 z-20 bg-black/60 text-white px-3 py-2 rounded-full block lg:hidden"
        >
          ❮
        </button>
        ...
        <button
          onClick={scrollRight}
          className="absolute right-2 top-1/2 -translate-y-1/2 z-20 bg-black/60 text-white px-3 py-2 rounded-full block lg:hidden"
        >
          ❯
        </button>
        ...
        <div
          ref={channelRef}
          className="flex gap-6 overflow-x-scroll scroll-smooth py-4 lg:overflow-x-hidden"
        >
          {channels.map((channel, index) => (
            <div key={index} className="flex flex-col items-center p-4 min-w-[100px]">
              ...
              <span className="font-semibold text-center">{channel.name}</span>
